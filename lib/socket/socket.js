...
import {asPlex, connect$, connectAndSend, createPlexPair, destroy, listenAndConnection$} from "rxprotoplex";
...
import {selectInterfaceByIp$} from "../network-interfaces/web-socket-network-interface.js";
import {interfacesEntitiesRef, onStoreReset$, socketEntitiesRef, store} from "../store.js";
...
export const selectSocketByIp$ = (ip) => store.pipe(
    selectEntityByPredicate(({ip: _ip}) => isIpInSubnet(_ip, ip), {ref: socketEntitiesRef})
);
 ...
 */
export const getSocketByIp = (ip) => store.query(getEntityByPredicate(o => isIpInSubnet(o.ip, ip), {ref: socketEntitiesRef}));
...
let localSocketInitialized = false;

export const enableLocalSocketPair = () => {
    if (localSocketInitialized) return; // Prevent multiple calls
    localSocketInitialized = true;

    const [server, client] = createPlexPair();

    onStoreReset$.pipe(take(1)).subscribe(() => {
        destroy(server);
        destroy(client);
        return localSocketInitialized = false;
    });
    ...
    store.update(
        addEntities([clientSocket, serverSocket], {ref: socketEntitiesRef}),
        updateEntities("lo", {server: serverSocket, client: clientSocket}, {ref: interfacesEntitiesRef})
    );

    console.debug("[enableLocalSocketPair] Localhost socket pair initialized.");
};
...
export const updateSocketIp = (socketId, socketIp) => {
    if (isLoopbackIp(socketIp) || socketId === "lo") {
        throw new Error("updateSocketIp doesn't work for loopback");
    }
...
export const setSocketConnected = (socketId) => {
    if (socketId.startsWith("lo")) return;
...
export const listenOnSocket$ = (fromIp, channel) => {
    ...
    if (isLoopbackIp(fromIp) || fromIp === "0.0.0.0") {
        const loSock = store.query(getEntity("lo_server", {ref: socketEntitiesRef}));
        if (loSock) {
            listeners.push(
                listenAndConnection$(loSock.plex, channel).pipe(
                    map(subSock => {
                        // Add metadata for local and remote IP addresses
                        subSock.localIp = "127.0.0.1";
                        subSock.remoteIp = "127.0.0.1";
                        return subSock;
                    })
                )
            );
    ...
    if (!isLoopbackIp(fromIp)) {
        listeners.push(
            store.pipe(
                // Filter sockets based on the IP address
                selectManyByPredicate(
                    sock => !!sock.ip && (isLoopbackIp(fromIp) || isIpInSubnet(sock.ifaceId, fromIp)),
                    {ref: socketEntitiesRef}
                ),
    ...
    return from(listeners).pipe(mergeAll());
};
...
export const sendMessage = (toIp, message, channel) => {
    if (isLoopbackIp(toIp)) {
        const loSock = store.query(getEntity("lo_client", {ref: socketEntitiesRef}));
        connectAndSend(loSock.plex, channel)(message);
        return;
    }
...
export const connectStream$ = (remoteIp, channel, localIp) => {
    if (isLoopbackIp(remoteIp)) {
        const clientLo = store.query(getEntity("lo_client", {ref: socketEntitiesRef}));
        if (clientLo) {
            return connect$(clientLo.plex, channel).pipe(
                map(subSock => {
                    // Add metadata for local and remote IP addresses
                    subSock.localIp = "127.0.0.1";
                    subSock.remoteIp = "127.0.0.1";
...
export const socketOfIpConnected$ = (ip) => {
    if (isLoopbackIp(ip)) {
        const sock = store.query(getEntity("lo_client", {ref: socketEntitiesRef}));
        return of(sock.client);
    }
